...
public final class LightningZapRequestEvent: NostrEvent {
    ...
    @available(*, unavailable, message: "This initializer is unavailable for LightningZapRequestEvent.")
    required init(
        kind: EventKind,
        content: String,
        tags: [Tag] = [],
    ...
    public init(
        ...
        tags.append(Tag(values: relayTagValues))
    ...
    public var relayURLs: [String] {
        return tags.first(where: { $0.name == "relays" })?.values.dropFirst().map(String.init) ?? []
    }
...
public final class LightningZapReceiptEvent: NostrEvent {
    ...
    @available(*, unavailable, message: "This initializer is unavailable for LightningZapReceiptEvent.")
    required init(
        kind: EventKind,
        content: String,
        tags: [Tag] = [],
